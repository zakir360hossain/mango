...
def game(category):
    ...
    current_set = sampled
    return render_template("game.html", data=sampled, current=sampled[0])


@app.route("/home/decide", methods=["GET"])
def game(category):
...
@app.route("/home/waiting_room", methods=["GET"])
def waiting_room():
...
def create(message):
    global host_id
    print(message)
    host_id = message["data"]["id"]
    emit("open_door", {"data": message.data}, broadcast=True)
